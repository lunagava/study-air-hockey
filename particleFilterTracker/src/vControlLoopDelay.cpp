...
//    fs.open("/data/iros_datasets/live_test/pf1.txt");
//    if(rf.check("file")) {
...
//    yarp::os::Network::connect(getName("/debug:o"), "/ptView", "fast_tcp");
    ...
    ptImg = cv::Mat::zeros(480,640,CV_8UC3);
    cv::namedWindow("ptView", cv::WINDOW_NORMAL);
    cv::imshow("ptView", cv::WINDOW_NORMAL);

    file2.open("/data/iros_datasets/exp2/vPFT/failure11.txt");
...
void delayControl::CallBackFunc(int event, int x, int y, int flags, void* param)
{
...
//        yarp::sig::ImageOf<yarp::sig::PixelBgr> &display= debug_port.prepare();
//        display = ptMap;
//        cv::Mat ptImg = cv::cvarrToMat((IplImage *)display.getIplImage());

        cv::circle(ptImg, cv::Point(avgx,avgy),5,cv::Scalar(14,127,255), -1);
        cv::rectangle(ptImg, cv::Point(qROI.getROI()[0],qROI.getROI()[2]),cv::Point(qROI.getROI()[1],qROI.getROI()[3]),cv::Scalar(14,127,255), 1);
//        image.resize(res.width, res.height);
//        image.zero();
//        m.lock();
////        drawcircle(image, avgx, avgy, avgr, 2);
//        drawTemplate(display, vpf.appearance, avgx, avgy, avgr);
//        drawEvents(image, qROI.q);
//        m.unlock();

        setMouseCallback("ptView", CallBackFunc, &pos_click);
        cv::imshow("ptView", ptImg);
        cv::waitKey(1);
...
//            yarp::sig::PixelBgr &ePix1 = ptMap.pixel((*a).x,(*a).y);
//            ePix1.r = ePix1.b = ePix1.g = 255;
    ...
    double currentT = yarp::os::Time::now();
...
//            yarp::sig::PixelBgr &ePix1 = ptMap.pixel((*a).x,(*a).y);
//            ePix1.r = ePix1.b = ePix1.g = 255;
            ptImg.at<cv::Vec3b>((*a).y, (*a).x) = cv::Vec3b(255,255,255);
...
//                double dT = yarp::os::Time::now() - currentT;
//                currentT += dT;
...
//                yInfo() << "Running at a cool " << 1.0 / dT << "Hz";
                ...
                if (fs.is_open())
                    data_to_save.push_back({a.timestamp() - time_offset, avgx, avgy, input_port.stats_unprocessed().duration});
            }
        }
        m.unlock();
