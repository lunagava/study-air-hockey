...
private:
    ...
    score_point convolution(cv::Mat eros, cv::Mat filter, double x_puck_pos, double y_puck_pos) {
        ...
        cv::Rect zoom = Rect(x_puck_pos - filter.cols * 0.5 - roi.x, y_puck_pos - filter.rows * 0.5 - roi.y, filter.cols+2,
...
//        yInfo()<<"ROI ="<<roi.x<<" "<<roi.y<<" "<<roi.width<<" "<<roi.height;
    ...
    void reset(cv::Point starting_position, int puck_size){
...
private:
    ...
    int n_seq;
    bool file_closed;
    yarp::os::BufferedPort<Bottle> puckPort;
